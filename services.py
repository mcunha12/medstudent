...
# Usamos st.cache_resource para manter a conexão aberta de forma eficiente
@st.cache_resource
...
def authenticate_or_register_user(email, password):
    """
    Autentica ou registra um usuário usando o banco de dados SQLite.
    """
    try:
        ...
        # 2. Se o usuário não existe, cria um novo (INSERT)
        ...
        # 3. Se o usuário existe, continua a verificação
        user_id = user_record['user_id'].iloc[0]
        stored_password_hash = user_record['password'].iloc[0]

        # 3a. Se não tiver senha, define a senha pela primeira vez (UPDATE)
        ...
        # 3b. Se já tiver senha, verifica se está correta
...
def save_answer(user_id, question_id, user_answer, is_correct):
    """
    Salva ou atualiza uma resposta no banco de dados SQLite.
    """
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        
        # 1. Verifica se já existe uma resposta para este usuário e questão
        ...
        # 2. Se não existe, insere uma nova resposta (INSERT)
        ...
        # 3. Se já existe, aplica a regra de atualização
        else:
            old_is_correct = str(existing_answer_df['is_correct'].iloc[0]).upper() == 'TRUE'
            
            # Atualiza apenas se a resposta antiga era um ERRO e a nova é um ACERTO
...
def get_simulado_questions(user_id, count=20, status_filters=['nao_respondidas'], specialty=None, provas=None, keywords=None):
    """
    Busca um lote de questões para um simulado a partir do banco de dados SQLite.
    """
    try:
        conn = get_db_connection()
        
        # Carrega as tabelas relevantes para a memória (muito mais rápido que gspread)
        questions_df = pd.read_sql_query("SELECT * FROM questions", conn)
        answers_df = pd.read_sql_query("SELECT * FROM answers WHERE user_id = ?", conn, params=(str(user_id),))

        if questions_df.empty: 
            return []

        # A lógica de filtragem em Pandas a partir daqui é a mesma de antes,
        # só que agora operando sobre dados lidos do SQLite.
        
        list_of_pools = []
        
        # Pool de questões não respondidas
        ...
        # Pools de questões corretas e incorretas
...
def _generate_concept_with_gemini(concept_name):
    """Gera a explicação de um conceito médico usando a API do Gemini. (Sem alterações)"""
    prompt = f"""
...
def get_performance_data(user_id):
    """Busca todos os dados de performance de um usuário usando o banco de dados SQLite."""
    try:
        conn = get_db_connection()
        
        # Usar JOIN em SQL é muito mais eficiente do que carregar duas tabelas e fazer merge
        query = """
        SELECT
            a.*,
            q.enunciado,
        ...
        # A partir daqui, o processamento do DataFrame com Pandas é o mesmo de antes
        ...
        # Para o ranking, precisamos de todas as respostas de todos os usuários
        ...
        # Tratamento das colunas de texto que podem ser listas
...
@st.cache_data(ttl=3600)
def get_all_provas():
    """Busca todas as provas únicas do SQLite."""
    try:
        conn = get_db_connection()
...
def get_global_platform_stats():
    """Calcula as estatísticas globais da plataforma a partir do SQLite."""
